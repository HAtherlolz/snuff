...
class AuthBackend(authentication.BaseAuthentication):
    ...
    def authenticate(self, request, token=None, **kwargs) -> Optional[tuple]:
        ...
        if len(auth_header) == 1:
            raise exceptions.AuthenticationFailed('Invalid token header. No credentioal provided.')
        elif len(auth_header) > 2:
            raise exceptions.AuthenticationFailed('Invalid token header. Token string should not contain space.')
